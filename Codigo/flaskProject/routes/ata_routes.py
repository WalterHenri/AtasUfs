...
from service import AtaService  # Assuming AtaService might be needed here eventually
...
# ata_service_instance = AtaService() # Better to get from app context or initialize once

@ata_bp.route('/')
def list_atas():
    try:
        # Fetch all Atas from the database
        atas_orm = Ata.query.order_by(Ata.created_at.desc()).all()
        return render_template('list_atas.html', atas=atas_orm)
    except Exception as e:
        logger.error(f"Erro ao listar ATAs: {e}")
        flash('Não foi possível carregar a lista de ATAs.', 'danger')
        return render_template('list_atas.html', atas=[])


@ata_bp.route('/new', methods=['GET', 'POST'])
def upload_ata():
    # Similar to chat_routes, ensure service is initialized properly
    if not hasattr(ata_bp, 'ata_service'):
        ata_bp.ata_service = AtaService()  # Initialize if not already
